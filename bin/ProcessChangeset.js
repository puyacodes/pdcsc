...
function scriptCopier(changeFile, dir) {
    const sbProcedures = [];
    const sbFunctions = [];
    const sbTables = [];
    const sbRelations = [];
    ...
    const files = getAllSqlFiles(dir); // فایل‌های SQL به‌صورت بازگشتی
...
function changes(basePath, changeset) {
    const changeFile = path.join(basePath, "Changes", `${changeset}.txt`);
    const scriptDir = path.join(basePath, "Scripts");

    const result = scriptCopier(changeFile, scriptDir);

    return result;
}

function processChangeset(basePath, filename) {
    return changes(basePath, filename);
}
