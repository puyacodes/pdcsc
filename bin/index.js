...
function extractChangesetItems(config, content) {
    ...
    for (let line of content.split("\n")) {
        ...
        if (base.isNullOrEmpty(line) && (!section || section == "customStart" || section == "customEnd")) {
            continue;
        }
...
function finalizeContent(config, content, sections, dropStatements) {
    ...
    if (!base.isEmpty(content) && content.split("\n").filter(line => line.trim().startsWith("##")).length > 0) {
        ...
        for (let line of content.split("\n")) {
            ...
            if (section) {
                if (!base.isSomeArray(draft[section])) {
...
async function renderChangesetScript(config, changesetPath, changesetName, deleteds, allFiles, appendAppVersion = true) {
    ...
    const hasAnything = !base.isEmpty(sb.customStart) ||
        !base.isEmpty(sb.customEnd) ||
        ...
        base.isSomeArray(sb.sequences) ||
        base.isSomeArray(sb.queues) ||
        base.isSomeArray(sb.assemblies) ||
        base.isSomeArray(sb.synonyms) ||
        base.isSomeArray(sb.statistics) ||
